...
'use strict';
var AqlError = require('./errors').AqlError, assumptions = require('./assumptions'), types = require('./types'), QB = {}, bind, toArray, warn;
bind = typeof Function.prototype.bind === 'function' ? Function.prototype.call.bind(Function.prototype.bind) : function (fn, self) {
    return function () {
...
SimpleReference.prototype.toAQL = function () {
    var value = String(this.value), tokens = value.split('.'), i;
...
function Definitions(dfns) {
    ...
    if (Object.prototype.toString.call(dfns) === '[object Array]') {
        for (var i = 0; i < dfns.length; i++) {
            ...
            this.dfns.push([
                new Identifier(dfns[i][0]),
                autoCastToken(dfns[i][1])
            ]);
        }
    } else {
        for (var key in dfns) {
...
function SortExpression(prev, args) {
    ...
    for (i = 0; i < args.length; i++) {
        ...
        if (!allowKeyword && value) {
            if (value instanceof Keyword || typeof value === 'string' && SortExpression.keywords.indexOf(value.toUpperCase()) !== -1) {
