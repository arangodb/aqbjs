...
describe('RemoveExpression', function () {
  ...
  it('auto-casts expressions', function () {
    ...
    for (var i = 0; i < arr.length; i++) {
      expect(new RemoveExpression(null, arr[i], 'y')._expr.constructor).to.equal(ctors[i]);
    }
  });
  ...
  it('wraps well-formed strings as collection names', function () {
    ...
    for (var i = 0; i < values.length; i++) {
      expect(new RemoveExpression(null, 'x', values[i])._collection.toAQL()).to.equal(values[i]);
    }
  });
  ...
  describe('options', function () {
    var expr = new RemoveExpression(null, 'x', 'y');
    it('returns a new RemoveExpression', function () {
      var optExpr = expr.options({a: 'b'});
      expect(optExpr).to.be.a(types.RemoveExpression);
      expect(optExpr.toAQL()).to.equal('REMOVE x IN y OPTIONS {a: b}');
    });
  });
